...
/**
 * xiaomi、huawei、vivo、oppo流行机型刘海屏适配工具类
 * 对某一个activity,需要在注册清单文件里给activity添加一个全屏theme
 *
 */
public class NotchScreenTool {
    ...
    /**
     * 获取状态栏高度
     *  ps://该方法可以作为获取vivo、oppo刘海屏、水滴屏的高度：官方没有给出标准的获取刘海高度的API，由于大多情况是：状态栏≥刘海，因此此处获取刘海高度采用状态栏高度
